function CGame(oData, iTeamChoosed) {
    ...
    var _bExtendedMatchForGoal = false;
    var _iTotScore;
    var _iLevelScore;
    var _iLevel;
    var _iPlayerTeam;
    ...
    this._init = function () {
        ...
        _oInterface = new CInterface(iTeamChoosed, _aOpponentsTeamVs[PLAYER_FLAG]);
    ...
    this._createPlayers = function () {
        ...
        for (var i = 0; i < PLAYERS_ON_FIELD; i++) {
            ...
            var iOpponentTeam = _aOpponentsTeamVs[PLAYER_FLAG];
            ...
            if (IS_HOST) {
                ...
                _aPlayers.push(_aPlayerUser[i] = this.createPlayer(oSpritePlayers, USER_PLAYERS[i].x, USER_PLAYERS[i].y, iID, -1, false, USER_PLAYERS[i].goal_keeper, iUserTeam, PLAYERS_USER, s_oScrollStage));
                iID++;
                _aPlayers.push(_aPlayerOpponent[i] = this.createPlayer(oSpritePlayers, OPPONENT_PLAYERS[i].x, OPPONENT_PLAYERS[i].y, iID, 1, true, OPPONENT_PLAYERS[i].goal_keeper, iOpponentTeam, PLAYERS_OPPONENT, s_oScrollStage));
                iID++;
            }
    ...
    window.socket.on('launchPenaltyBy', async (data) => {
        _iNumOfShot = 0;
        _iPlayerIDSelect = -1;
        _oArrow.setVisible(false);
    ...
    window.socket.on("kickBy", async (data) => {
        if (!_bCpuTurn) {
            var vHitDir = new CVector2(data.point1 - data.point2,
                data.point3 - data.point4);
            ...
            if (fForceLength > HIT_PLAYER_MIN_FORCE) {
                ...
                s_oGame.addForceToPlayer(_aPlayers[data.playerID], vHitDir);
    ...
    function sendKickData(playerID, point1, point2, point3, point4, force) {
        var data = {
            roomID: ROOM_ID,
            playerID: playerID,
            point1: point1,
    ...
    this.onPressUp = function () {
        ...
        sendKickData(_iPlayerIDSelect, _aPlayers[_iPlayerIDSelect].getX(), _oReleasePoint.x, _aPlayers[_iPlayerIDSelect].getY(), _oReleasePoint.y, _fForce);
    ...
    this.getOpponentTeam = function () {
        return _aOpponentsTeamVs[PLAYER_FLAG];
    };
    ...
    this.storesResult = function () {
        _aResults[_iLevel] = {
            player_team: _iPlayerTeam, opponent_team: _aOpponentsTeamVs[_iLevel],
    ...
    this.goal = function (iGoalSide) {
        ...
        if (_bExtendedMatchForGoal === true) {
            this.finishTime()
        }
    };
    ...
    this.launchDelayPenality = function (fSecond) {
        ...
        if (_fTimePenality > 0) {
            ...
            window.socket.emit("launchPenalty", data)
        }
